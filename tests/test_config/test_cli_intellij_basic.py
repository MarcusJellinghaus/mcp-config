...
from src.mcp_config.main import handle_setup_command, handle_remove_command, handle_list_command
from src.mcp_config.cli_utils import parse_and_validate_args, SUPPORTED_CLIENTS
...
class TestIntelliJCLIBasic:
    ...
    def test_intellij_cli_argument_parsing(self) -> None:
        ...
        args, errors = parse_and_validate_args([
            "setup", "mcp-code-checker", "test-server", 
            "--project-dir", ".", "--client", "intellij"
        ])
    ...
    def test_intellij_remove_command_basic(
        ...
        mock_client.list_managed_servers.return_value = [
            {"name": "test-server", "type": "mcp-code-checker", "command": "python", "managed": True}
        ]
        mock_get_client.return_value = mock_client
    ...
    def test_intellij_list_command_basic(
        ...
        mock_client.list_all_servers.return_value = [
            {"name": "server1", "type": "mcp-code-checker", "command": "python", "managed": True},
            {"name": "server2", "type": "custom-server", "command": "node", "managed": False},
        ]
        mock_get_client.return_value = mock_client
    ...
    def test_intellij_dry_run_setup(
        ...
        with patch('src.mcp_config.main.registry') as mock_registry, \
             ...
             patch('src.mcp_config.main.validate_parameter_combination') as mock_validate_combo, \
            ...
            args, errors = parse_and_validate_args([
                "setup", "mcp-code-checker", "test-server",
                "--project-dir", ".", "--client", "intellij", "--dry-run"
            ])
    ...
    def test_intellij_validation_errors_handled(self) -> None:
        ...
        with pytest.raises(SystemExit):
            parse_and_validate_args([
                "setup", "mcp-code-checker", "test-server", "--client", "intellij"
...
class TestIntelliJCLIIntegrationEnd2End:
    ...
    def test_intellij_full_setup_workflow(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            args, errors = parse_and_validate_args([
                "setup", "mcp-code-checker", "test-server",
                "--project-dir", temp_dir, "--client", "intellij"
            ])
