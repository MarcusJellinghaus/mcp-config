...
class TestClaudeDesktopHandler:
    ...
    def clean_real_config(self):
        ...
        try:
            ...
            if real_config_path.exists():
                ...
                test_server_names = {'my-server', 'my_server', 'test-server', 'server1', 'server2'}
                if 'mcpServers' in real_config:
                    polluted_servers = set(real_config['mcpServers'].keys()) & test_server_names
                    if polluted_servers:
                        # Create backup with timestamp
                        import datetime
                        timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
                        backup_path = real_config_path.parent / f"test_backup_{timestamp}.json"
    ...
    def mock_config_path(self, temp_config_dir: Path, monkeypatch: Any) -> None:
    ...
    def test_load_missing_config(self, handler: ClaudeDesktopHandler, monkeypatch: Any) -> None:
        ...
        assert "tmp" in str(config_path) or "temp" in str(config_path).lower(), f"Expected temp path, got: {config_path}"
