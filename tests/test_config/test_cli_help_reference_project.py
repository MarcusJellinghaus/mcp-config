...
def test_reference_project_parameter_in_help() -> None:
    ...
    result = subprocess.run(
        [
            sys.executable, "-m", "src.mcp_config.main",
            "setup", "mcp-server-filesystem", "--help"
        ],
        capture_output=True,
        text=True,
        cwd=Path.cwd()
    ...
    assert "repeatable" in help_text or "multiple times" in help_text or "append" in help_text
...
def test_help_text_user_friendly() -> None:
    ...
    assert ("multiple" in help_text.lower() or 
            "repeatable" in help_text.lower() or 
            "times" in help_text.lower())
...
def test_cli_parser_handles_multiple_reference_projects() -> None:
    ...
    args = parser.parse_args([
        "setup", "mcp-server-filesystem", "test-server",
